...
//if you staked more times you get more rewards
rule moreTokensMoreTimeStaked(address user1) {
   uint amount;
   method f;
   ...
   require e1.msg.sender == user;
   stake(e1,amount);
   uint256 rewardsAmountBefore = earned(e1, user);

   env ef;
   require ef.msg.sender != user;
   calldataarg args;
   f(ef, args);

   env e2;
   require e2.block.timestamp > e1.block.timestamp;
   uint256 rewardsAmountAfter = earned(e2, user);

    assert rewardsAmountBefore < rewardsAmountAfter, "It was called later";
}
