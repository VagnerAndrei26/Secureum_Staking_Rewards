...
hook Sstore rewards[KEY address addr] uint256 newRewards (uint256 oldReward) STORAGE {
    sumOfRewards = sumOfRewards + newRewards - oldReward;
}

hook Sload uint256 rew balanceOf[KEY address addr] STORAGE {
...
//Sum of rewards is atleast equal to stakingtoken.balanceOf(address(this))
invariant sumOfRewardsForStakingToken()
...
rule changeBalanceOfUserOnSomeMethods(env e, uint256 amount, address user) {
...
//State transition
//totalSupply is 0 if the stakingToken.balanceOf(address(this)) is 0
invariant totalSupplyToBalanceOf()
    totalSupply() > 0 => stakingToken.balanceOf(currentContract) > 0
    {
...
//Monotonocity between finishAt and rewardRate
invariant monotonocityBetweenFinishAtAndRewardRate()
