...
//Staking tokens should shouldn't decrease the rewards
rule moreTokensStakedMoreRewards(env e1, address user) {
    ...
    assert amountEarnedBefore <= amountEarnedAfter, "Less rewards after the user staked tokens";
}

//Person who staked the tokens more time , has greater rewards
rule moreTokensMoreTimeStaked(address user1, address user2) {
    uint amount;
    ...
    uint256 amountEarnedStake1 = rewards(user1);
    uint256 amountEarnedStake2 = rewards(user2);

    assert amountEarnedStake1 > amountEarnedStake2 => e1.block.timestamp < e2.block.timestamp, "It was called later";
}
