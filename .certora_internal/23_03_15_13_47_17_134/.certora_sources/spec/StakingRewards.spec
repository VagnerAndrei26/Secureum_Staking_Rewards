...
rule userCanAlwaysUnstakeAfterStake(env e1, env e2, address user, uint256 amount) {
    method f;
    calldataarg args;
    firstSetup(e1, user);
    stake(e1,amount);
    uint256 amountBeforeWithdraw = balanceOf(user);
    f(e2,args);
    withdraw(e1,amount);
    uint256 amountAfterWithdraw = balanceOf(user);
