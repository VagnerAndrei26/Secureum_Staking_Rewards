...
rule moreTokensStakedMoreRewards(env e, address user, address user2) {
    ...
    stake(e, amount);
    require e.msg.sender == user2;
    stake(e, amount2);

    uint256 rewardsTokens = earned(e, user);
    uint256 rewardsTokens2 = earned(e, user2);
 
    assert amount > amount2 => rewardsTokens > rewardsTokens2, "Less rewards for the user with greater balance";
}
