...
rule moreStakeMoreRewards(address user1, address user2) {
    ...
    uint256 rewardsBefore1 = rewards(user1);
    uint256 rewardsBefore2 = rewards(user2);
    uint256 balance1 = balanceOf(user1);
    uint256 balance2 = balanceOf(user2);

    updateRewardHelper(e, user1);
    uint256 rewardsAfter1 = rewards(user1);

    updateRewardHelper(e, user2) at initial;
    uint256 rewardsAfter2 = rewards(user2);

    
    assert (rewardsAfter1 - rewardsBefore1 < rewardsAfter2 - rewardsBefore2) =>
           (balance1 < balance2),
           "User with more tokens staked should get more rewards token";
}

//BalanceOf(user) and rewards(user) should be in anti-monotonicity with staking.balanceOf(user) and rewardsToken.balanceOf(user) respectively
rule antiMonotonicity(address user) {
    env e; method f; calldataarg args;
    firstSetup(e, user);
    uint256 userBalanceBefore = balanceOf(user);
    uint256 userStakingTokensBefore = stakingToken.balanceOf(user);
    uint256 userRewardsBefore = rewards(user);
    uint256 userRewardsTokensBefore = rewardsToken.balanceOf(user);
    f(e, args);
    uint256 userBalanceAfter = balanceOf(user);
    uint256 userStakingTokensAfter = stakingToken.balanceOf(user);
    uint256 userRewardsAfter = rewards(user);
    uint256 userRewardsTokensAfter = rewardsToken.balanceOf(user);
    assert userBalanceBefore < userBalanceAfter <=> 
            userStakingTokensBefore > userStakingTokensAfter,
            "BalanceOf(user) and stakingToken.balanceOf(user) are not in anti-monotonicity";
    assert userRewardsBefore > userRewardsAfter =>
            userRewardsTokensBefore < userRewardsTokensAfter,
            "Rewards(user) and rewardsToken.balanceOf(user) are not in anti-monotonicity";
}
