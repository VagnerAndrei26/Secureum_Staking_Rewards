...
rule rewardsModification(env e,method f, address user) {
    require user != owner() && user != currentContract && user != 0 && user == e.msg.sender;
    require f.selector != updateRewardHelper(address).selector;
    ...
    f.selector == getReward().selector) , "Rewards was modified on something else then the specified methods";
}
...
rule rewardsShouldntDecrease(env e, address user,uint256 amount) {
    ...
    // require totalSupply() > 0 && finishAt() > 0 && rewardRate() > 0;
...
//     assert e3.block.timestamp > finishAt() => rewardsAmountBeforeFirstStake < rewardsAmountAfterWithdraw, "It was called later";
// }
