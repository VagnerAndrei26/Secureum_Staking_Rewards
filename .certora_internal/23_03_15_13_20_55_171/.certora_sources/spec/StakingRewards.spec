...
//No function should decrease the rewards of an user if getRewards was not called;
rule rewardsShouldntDecrease(env e, address user,uint256 amount) {
    ...
    require f.selector != getReward().selector;
    f(e,args);
    uint256 amountEarnedAfter = rewards(user);

 
    assert (amountEarnedBefore <= amountEarnedAfter), "Rewards decreased on other methods than getRewards";
}
