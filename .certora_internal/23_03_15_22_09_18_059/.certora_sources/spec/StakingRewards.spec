...
function global_requires(env e) {
    require stakingToken() == stakingToken;
    require rewardsToken() == rewardsToken;
    require e.msg.sender != currentContract;
...
rule nonDecreasingVariables(method f, address user) {
    env e1; env e2; calldataarg args1; calldataarg args2;
    ...
    mathint finishAtBefore = finishAt();
    mathint updatedAtBefore = updatedAt();
    mathint rewardPerTokenStoredBefore = rewardPerTokenStored();
    mathint userRewardPerTokenPaidBefore = userRewardPerTokenPaid(user);

    f(e2, args2);

    mathint finishAtAfter = finishAt();
    mathint updatedAtAfter = updatedAt();
    mathint rewardPerTokenStoredAfter = rewardPerTokenStored();
    mathint userRewardPerTokenPaidAfter = userRewardPerTokenPaid(user);

    assert finishAtAfter >= finishAtBefore, "finishAt must be non-decreasing";
    assert updatedAtAfter >= updatedAtBefore, "updatedAt must be non-decreasing";
    assert rewardPerTokenStoredAfter >= rewardPerTokenStoredBefore, "rewardPerTokenStored must be non-decreasing";
    assert userRewardPerTokenPaidAfter >= userRewardPerTokenPaidBefore, "userRewardPerTokenPaid[user] must be non-decreasing";
}

rule moreStakeMoreRewards(address user1, address user2) {
    env e; storage initial = lastStorage;

    global_requires(e);

    // Neither users are the zero address
    ...
    mathint user1RewardsBefore = rewards(user1);
    mathint user2RewardsBefore = rewards(user2);
    mathint user1Balance = balanceOf(user1);
    mathint user2Balance = balanceOf(user2);

    updateRewardHelper(e, user1);
    mathint user1RewardsAfter = rewards(user1);

    updateRewardHelper(e, user2) at initial;
    mathint user2RewardsAfter = rewards(user2);
