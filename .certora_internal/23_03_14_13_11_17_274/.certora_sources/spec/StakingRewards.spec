...
invariant totalSupplyToBalanceOf(env e)
    totalSupply() == 0 <=> balanceOf(e.msg.sender) == 0


//Monotonocity between rewards(user) and balanceOf(user)
invariant monotonicityRewardsBalanceOf(address user)
    rewards(user) > 0 => balanceOf(user) > 0