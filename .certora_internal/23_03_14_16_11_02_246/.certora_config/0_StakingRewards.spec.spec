...
rule moreTokensStakedMoreRewards(address user, address user2) {
    uint256 amount;
    uint256 amount2;
    ...
    uint256 rewardsTokens = earned(e1,user);
    uint256 rewardsTokens2 = earned(e2,user2);
 
    assert (amount > amount2 && e1.block.timestamp > e2.block.timestamp) => rewardsTokens > rewardsTokens2, "Less rewards for the user with greater balance";
}
